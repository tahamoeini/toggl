...
def getWorkspace():
    try:
        ...
        return list(workspacesIds)[position]
...
def getProjects():
    ...
    projects = requests.get(
        apiUrl + "workspaces/" + str(workspaceId) + "/projects", auth=(key, "api_token")
    ).json()
...
def getTimeEntries():
    timeEntries = requests.get(
        apiUrl
        + urllib.parse.quote(
            "time_entries?start_date=" + startDate + "&end_date=" + endDate, safe="=?&"
        ),
        auth=(key, "api_token"),
    ).json()

    timeMonthEntries = requests.get(
        apiUrl
        + urllib.parse.quote(
            "time_entries?start_date=" + startMonthDate + "&end_date=" + endDate,
            safe="=?&",
        ),
        auth=(key, "api_token"),
    ).json()

    return timeEntries, timeMonthEntries


timeEntries, timeMonthEntries = getTimeEntries()


def getTimeLimitation():
    totalWeekTime = 0
        ...
        for project in projectsList.values():
            ...
            if maxDurationOfProjects[project.name] < project.duration / 3600:
                f.write(
                    project.name
                    + ": "
                    + str(math.ceil(project.duration / 360) / 10)
                    + "h per week - OverDo! with "
                    + str(
                        math.ceil(
                            (
                                project.duration / 3600
                                - maxDurationOfProjects[project.name]
                            )
                            * 10
                        )
            ...
            if maxDurationOfProjects[project.name] * 4 < project.monthDuration / 3600:
                f.write(
                    project.name
                    + ": "
                    + str(math.ceil(project.monthDuration / 360) / 10)
                    + "h per month - OverDo! with "
                    + str(
                        math.ceil(
                            (
                                project.monthDuration / 3600
                                - maxDurationOfProjects[project.name] * 4
                            )
                            * 10
                        )
                        / 10
                ...
                f.write(
                    ...
                    + ": "
                    + str(math.ceil(project.monthDuration / 360) / 10)
                    + "h per month - Work More! You need "
                    + str(
                        math.ceil(
                            (
                                minDurationOfProjects[project.name] * 4
                                - project.monthDuration / 3600
                            )
                            * 10
                        )
    ...
    return totalWeekTime, totalMonthTime


totalWeekTime, totalMonthTime = getTimeLimitation()
...
def createGraph():
    ...
    week.pie(
        chartData, labels=chartProjects, colors=colors, autopct="%1.1f%%", startangle=90
    )
    month.pie(
        chartMonthData,
...
def exportICal():
    ...
    for timeEntry in timeEntries:
        ...
        event = Event()
        if project:
            event.add("summary", project.name)
            print(str(project.name))
        else:
            event.add("summary", "unknown")
        startDate = tuple([int(x) for x in timeEntry["start"][:10].split("-")]) + tuple(
            [int(x) for x in timeEntry["start"][12:18].split(":")]
        )
        startDate = datetime.datetime(*startDate) + datetime.timedelta(seconds=16200)
        event.add("dtstart", startDate)
        print(str(startDate))
        stopDate = startDate + datetime.timedelta(seconds=timeEntry["duration"])
        if stopDate.day > startDate.day:
            stopDate += datetime.timedelta(seconds=86400)
        event.add("dtend", stopDate)
        print(str(stopDate))
        event.add("description", timeEntry["description"])
        print(str(timeEntry["description"]) + "\n")
        cal.add_component(event)

    f = open("times_cal.ics", "wb")
    f.write(cal.to_ical())
    f.close()
